...
const SOURCE_DIR = path.resolve('./packages/react-hooks/src')
const GITHUB_REPO = `tuannafed/7uan`
...
const hooks = getHooks()
  ...
  .map(hook => {
    const url = `https://github.com/${GITHUB_REPO}/tree/master/packages/react-hooks/src/${hook.name}`
    return {
      ...hook,
...
3. Write tests for the specific hook in \`packages/react-hooks/src/${hook.name}/useExample.test.ts\`.
